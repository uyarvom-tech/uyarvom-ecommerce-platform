      ...
      <div className="bg-white border rounded-none overflow-hidden shadow-sm">
            ...
            {filteredProducts.map((product) => (
              <tr key={product.id} className="hover:bg-muted/10 transition-colors group">
                ...
                <td className="px-6 py-6">
                  ...
                  <p className="text-[8px] font-bold uppercase text-muted-foreground opacity-60">Base price</p>
                </td>
                <td className="px-6 py-6">
                ...
                <td className="px-6 py-6 text-right">
                  <div className="flex gap-2 justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => router.push(`/admin/products/${product.id}/edit`)} title="Edit product" className="p-2 hover:bg-black hover:text-white transition-all"><Edit className="h-4 w-4" /></button>
                    <button onClick={() => router.push(`/products/${product.slug}`)} title="View product" className="p-2 hover:bg-primary transition-all"><ArrowUpRight className="h-4 w-4" /></button>
                  </div>
                </td>
              </tr>
            ))}
        ...
        {filteredProducts.length === 0 && (
          <div className="py-24 text-center">
            ...
            <p className="text-[10px] font-black uppercase tracking-[.3em] text-muted-foreground">No products found</p>
          </div>
        )}
      </div>
    </div>
